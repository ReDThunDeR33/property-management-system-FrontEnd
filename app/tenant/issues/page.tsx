...
export default function IssuesPage() {
  ...
  useEffect(() => {
    const fetchIssues = async () => {
      ...
      try {
        const response = await api.get(`/tenant/${tenantId}/issues`);
        ...
        <div className="flex items-start justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-2">
              Track issues reported for your assigned property.
            </p>
          </div>

          <Link
            href="/tenant/issues/new"
            className="bg-[#FF5A3D] text-white px-5 py-2.5 rounded-lg hover:bg-[#e94e34] transition"
          >
            + Report Issue
          </Link>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredIssues.map((issue) => (
              <Link
                ...
                className="bg-white border border-gray-200 rounded-xl p-6 hover:border-[#FF5A3D] transition"
              >
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold">Issue #{issue.id}</h3>

                  <span
                    className={`text-xs px-2 py-1 rounded-full ${statusStyle[issue.status]}`}
                  >
